...
searchButton.addEventListener('click', () => {
    ...
    if (searchValue) {
        filterProductsByFirstLetter(searchValue);
    } else {
        addDataToHTML(products);
    }
})

const filterProductsByFirstLetter = (letter) => {
    let filteredProducts = products.filter(product => product.name.toLowerCase().startsWith(letter));
...
const addDataToHTML = (productList) => {
    ...
    // add new products
    if (productList.length > 0) { // if there are products
